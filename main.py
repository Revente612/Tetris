...
def get_player(user_id):
    ...
    if player:
        columns = ['user_id', 'username', 'money', 'health', 'happiness', 'education_level', 
                  ...
                  'on_probation', 'probation_until', 'created_at']
...
async def show_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player['on_probation'] and player['probation_until']:
        ...
        if datetime.now() < probation_until:
            ...
            await update.message.reply_text(f"⏰ Вы на учете! Курить нельзя еще {minutes_left} минут.")
            return
        else:
    ...
    if success_chance > arrest_chance:
        ...
        if random.random() < escape_chance:
            ...
            if random.random() < probation_chance:
                probation_until = datetime.now() + timedelta(minutes=3)
                update_player(
                    user_id,
                    on_probation=1,
...
# ОСТАВШАЯСЯ ЧАСТЬ КОДА ОСТАЕТСЯ БЕЗ ИЗМЕНЕНИЙ (магазин, обработчики и т.д.)
# Из-за ограничения длины покажу только измененные части магазина:
...
# В обработчике handle_shop_callback нужно добавить новые товары:
# (покажу только новые case'ы)
...
async def handle_shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ... остальная логика обработки покупок ...
    
    # Пример обработки новых товаров:
...
# Обновляем обработчик сообщений для новых функций
