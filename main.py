...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS players (
            ...
            apartment_type TEXT DEFAULT NULL,
            apartment_comfort INTEGER DEFAULT 0,
            ...
            girlfriend_name TEXT DEFAULT NULL,
            ...
            car_condition INTEGER DEFAULT 100,
...
def upgrade_db():
    ...
    try:
        ...
        new_columns = {
            'apartment_type': 'ALTER TABLE players ADD COLUMN apartment_type TEXT DEFAULT NULL',
            'apartment_comfort': 'ALTER TABLE players ADD COLUMN apartment_comfort INTEGER DEFAULT 0',
            'girlfriend_name': 'ALTER TABLE players ADD COLUMN girlfriend_name TEXT DEFAULT NULL',
            'phone_type': 'ALTER TABLE players ADD COLUMN phone_type TEXT DEFAULT NULL',
            'car_condition': 'ALTER TABLE players ADD COLUMN car_condition INTEGER DEFAULT 100',
            'bank_balance': 'ALTER TABLE players ADD COLUMN bank_balance INTEGER DEFAULT 0',
            'last_apartment_payment': 'ALTER TABLE players ADD COLUMN last_apartment_payment TEXT'
        }
...
def get_player(user_id):
    ...
    if player:
        columns = ['user_id', 'username', 'money', 'health', 'happiness', 'education_level', 
                  'job_level', 'has_apartment', 'apartment_type', 'apartment_comfort', 'has_laptop', 'has_vpn', 'food', 'cigarettes',
                  'vape_type', 'vape_juice', 'juice_flavor', 'snus_packs', 'snus_strength',
                  'has_girlfriend', 'girlfriend_happiness', 'girlfriend_name', 'age', 'has_id', 'last_work_time',
                  ...
                  'has_iphone', 'has_samsung', 'phone_type', 'parents_angry', 'parents_angry_until', 'police_in_school', 
                  ...
                  'has_car', 'car_type', 'car_condition', 'insurance', 'loan', 'loan_debt', 'savings', 'investments',
                  'bank_balance', 'last_apartment_payment']
        ...
        defaults = {
            'apartment_comfort': 0, 'girlfriend_name': None, 'phone_type': None,
            'car_condition': 100, 'bank_balance': 0, 'friends_count': 0,
            'family_happiness': 0, 'children': 0, 'has_family': 0
        }
...
PHONES = {
    "iPhone 15 Pro": {"price": 1000, "prestige": 10},
    "Samsung Galaxy S24": {"price": 800, "prestige": 8},
    "Xiaomi Redmi Note": {"price": 300, "prestige": 3},
    "Huawei P60": {"price": 600, "prestige": 6}
}

APARTMENTS = {
    "Комната в общежитии": {"price": 500, "rent": 50, "comfort": 2},
    "Однокомнатная квартира": {"price": 2000, "rent": 200, "comfort": 5},
    "Двухкомнатная квартира": {"price": 4000, "rent": 400, "comfort": 8},
    "Пентхаус": {"price": 10000, "rent": 1000, "comfort": 15}
}

CARS = {
    "Lada Granta": {"price": 1000, "condition": 80, "prestige": 2},
    "Kia Rio": {"price": 3000, "condition": 90, "prestige": 5},
    "BMW X5": {"price": 8000, "condition": 95, "prestige": 10},
    "Mercedes S-Class": {"price": 15000, "condition": 98, "prestige": 15}
}
...
GIRL_NAMES = ["Анна", "Мария", "Екатерина", "София", "Дарья", "Виктория", "Алиса", "Полина", "Ольга", "Ирина"]
...
🏡 Квартира: {'✅ ' + player['apartment_type'] if player['has_apartment'] else '❌ Нет'}
🚗 Машина: {'✅ ' + player['car_type'] if player['has_car'] else '❌ Нет'}
📱 Телефон: {'✅ ' + player['phone_type'] if player['phone_type'] else '❌ Нет'}
...
"""
    ...
    if player['has_girlfriend']:
        status_text += f"\n💕 Девушка: {player['girlfriend_name']} ({player['girlfriend_happiness']}/100 счастья)"
...
async def phones_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for name, info in PHONES.items():
        text += f"{name} - {info['price']} руб. (престиж: +{info['prestige']})\n"
    
    keyboard = []
    for name in PHONES.keys():
        keyboard.append([InlineKeyboardButton(f"Купить {name}", callback_data=f"buy_phone_{name}")])
    keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")])
...
async def apartment_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player['has_apartment']:
        text += f"У вас: {player['apartment_type']}\n"
        text += f"Комфорт: {player['apartment_comfort']}/10\n\n"
        text += "Доступные действия:"
        keyboard = [
            [InlineKeyboardButton("💵 Оплатить аренду", callback_data="pay_rent")],
            [InlineKeyboardButton("🛋️ Улучшить комфорт", callback_data="improve_comfort")],
            [InlineKeyboardButton("🏠 Купить новую квартиру", callback_data="buy_apartment")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
        ]
    else:
        text += "У вас нет жилья!\n\nДоступные варианты:"
        keyboard = []
        for name, info in APARTMENTS.items():
            keyboard.append([InlineKeyboardButton(f"🏠 {name} - {info['price']} руб.", callback_data=f"buy_apartment_{name}")])
        keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")])
...
async def girlfriend_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player['has_girlfriend']:
        text = f"💕 Ваша девушка: {player['girlfriend_name']}\n"
        text += f"Счастье: {player['girlfriend_happiness']}/100\n\n"
        text += "Доступные действия:"
        keyboard = [
            [InlineKeyboardButton("💑 Свидание", callback_data="date_girlfriend")],
            [InlineKeyboardButton("🎁 Подарок", callback_data="gift_girlfriend")],
            [InlineKeyboardButton("💔 Расстаться", callback_data="breakup_girlfriend")],
            [InlineKeyboardButton("👨‍👩‍👧‍👦 Создать семью", callback_data="start_family")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
        ]
    else:
        text = "💕 У вас нет девушки\n\n"
        text += "Хотите найти девушку?"
        keyboard = [
            [InlineKeyboardButton("💕 Найти девушку", callback_data="find_girlfriend")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
        ]
...
async def transport_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player['has_car']:
        text += f"У вас: {player['car_type']}\n"
        text += f"Состояние: {player['car_condition']}%\n\n"
        text += "Доступные действия:"
        keyboard = [
            [InlineKeyboardButton("🔧 Починить машину", callback_data="repair_car")],
            [InlineKeyboardButton("🚗 Купить новую машину", callback_data="buy_car_menu")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
        ]
    else:
        text += "У вас нет машины!\n\nДоступные варианты:"
        keyboard = []
        for name, info in CARS.items():
            keyboard.append([InlineKeyboardButton(f"🚗 {name} - {info['price']} руб.", callback_data=f"buy_car_{name}")])
        keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")])
...
async def bank_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🏦 БАНК\n\n"
    text += f"💵 На счету: {player['bank_balance']} руб.\n"
    text += f"💰 Сбережения: {player['savings']} руб.\n"
    text += f"💸 Долг: {player['loan_debt']} руб.\n\n"
    
    keyboard = [
        [InlineKeyboardButton("💳 Положить на счет", callback_data="deposit_money")],
        [InlineKeyboardButton("🏧 Снять со счета", callback_data="withdraw_money")],
        [InlineKeyboardButton("💰 Взять кредит", callback_data="take_loan")],
        [InlineKeyboardButton("💵 Погасить долг", callback_data="pay_debt")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
    ]
...
async def health_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += f"🦷 Зубы: {player['dental_health']}/100\n"
    text += f"🌟 Кожа: {player['skin_health']}/100\n\n"
    
    if player['has_lung_cancer']:
        text += "⚠️ У вас РАК ЛЕГКИХ!\n"
    if player['has_heart_disease']:
        text += "💔 У вас БОЛЕЗНИ СЕРДЦА!\n"
    
    keyboard = [
        [InlineKeyboardButton("👨‍⚕️ Посетить врача", callback_data="visit_doctor")],
        [InlineKeyboardButton("💊 Лечение", callback_data="treatment_menu")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
    ]
...
async def skills_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += f"💪 Сила воли: {player['willpower']}/100\n"
    text += f"🧠 Интеллект: {player['intelligence']}/100\n"
    text += f"💼 Харизма: {player['charisma']}/100\n"
    text += f"🏃 Выносливость: {player['endurance']}/100\n\n"
    
    keyboard = [
        [InlineKeyboardButton("💪 Сила воли (300 руб.)", callback_data="improve_willpower")],
        [InlineKeyboardButton("🧠 Интеллект (400 руб.)", callback_data="improve_intelligence")],
        [InlineKeyboardButton("💼 Харизма (350 руб.)", callback_data="improve_charisma")],
        [InlineKeyboardButton("🏃 Выносливость (300 руб.)", callback_data="improve_endurance")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")]
    ]
...
async def handle_shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif query.data.startswith("buy_phone_"):
        await handle_phone_purchase(query)
    
    elif query.data.startswith("buy_apartment_"):
        await handle_apartment_purchase(query)
    
    elif query.data.startswith("buy_car_"):
        await handle_car_purchase(query)
    
    elif query.data.startswith("improve_"):
        await handle_skill_improvement(query)
    
    elif query.data == "find_girlfriend":
        await find_girlfriend(query)
    
    elif query.data == "date_girlfriend":
        await date_girlfriend(query)
    
    elif query.data == "gift_girlfriend":
        await gift_girlfriend(query)
...
async def handle_phone_purchase(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    data = query.data
    
    phone_name = data.replace("buy_phone_", "")
    if phone_name in PHONES:
        info = PHONES[phone_name]
        ...
        update_player(
            ...
            has_iphone=1 if "iPhone" in phone_name else 0,
            has_samsung=1 if "Samsung" in phone_name else 0
        )
        
        await query.answer(f"✅ Куплен {phone_name}!", show_alert=True)
        await query.edit_message_text(f"✅ Вы купили {phone_name} за {info['price']} руб.\n📱 Престиж: +{info['prestige']}")

async def handle_apartment_purchase(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    data = query.data
    
    if data == "buy_apartment":
        # Показать список квартир
        text = "🏡 ВЫБЕРИТЕ КВАРТИРУ:\n\n"
        for name, info in APARTMENTS.items():
            text += f"{name} - {info['price']} руб. (аренда: {info['rent']}/мес)\n"
        
        keyboard = []
        for name in APARTMENTS.keys():
            keyboard.append([InlineKeyboardButton(f"🏠 {name}", callback_data=f"buy_apartment_{name}")])
        keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="back_to_main")])
        
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        return
    
    apartment_name = data.replace("buy_apartment_", "")
    if apartment_name in APARTMENTS:
        info = APARTMENTS[apartment_name]
        ...
        update_player(
            ...
            apartment_comfort=info['comfort'],
            last_apartment_payment=datetime.now().isoformat()
        )
        
        await query.answer(f"✅ Куплена {apartment_name}!", show_alert=True)
        await query.edit_message_text(f"✅ Вы купили {apartment_name} за {info['price']} руб.\n🏠 Комфорт: {info['comfort']}/10")

async def handle_car_purchase(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    data = query.data
    
    car_name = data.replace("buy_car_", "")
    if car_name in CARS:
        info = CARS[car_name]
        ...
        update_player(
            ...
            has_car=1,
            car_type=car_name,
            car_condition=info['condition']
        )
        
        await query.answer(f"✅ Куплена {car_name}!", show_alert=True)
        await query.edit_message_text(f"✅ Вы купили {car_name} за {info['price']} руб.\n🚗 Состояние: {info['condition']}%")

async def handle_skill_improvement(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    data = query.data
    
    skill_costs = {
        "improve_willpower": {"cost": 300, "skill": "willpower", "name": "💪 Сила воли"},
        "improve_intelligence": {"cost": 400, "skill": "intelligence", "name": "🧠 Интеллект"},
        "improve_charisma": {"cost": 350, "skill": "charisma", "name": "💼 Харизма"},
        "improve_endurance": {"cost": 300, "skill": "endurance", "name": "🏃 Выносливость"}
    }
    
    if data in skill_costs:
        skill_info = skill_costs[data]
        ...
        current_skill = player[skill_name]
        if current_skill >= 100:
            await query.answer("❌ Навык уже максимального уровня!", show_alert=True)
            return
        
        improvement = random.randint(5, 15)
        new_skill = min(100, current_skill + improvement)
        ...
        await query.answer(f"✅ {skill_info['name']} улучшена!", show_alert=True)
        await query.edit_message_text(
            f"✅ Навык улучшен!\n\n"
            f"{skill_info['name']}: +{improvement} (теперь {new_skill}/100)\n"
            f"💵 Потрачено: {cost} руб.\n"
            f"💰 Осталось: {player['money'] - cost} руб."
        )

# СИСТЕМА ОТНОШЕНИЙ
async def find_girlfriend(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    
    if player['has_girlfriend']:
        await query.answer("❌ У вас уже есть девушка!", show_alert=True)
        return
    
    if player['charisma'] < 30:
        await query.answer("❌ Слишком низкая харизма! Поднимите до 30.", show_alert=True)
        return
    
    girlfriend_name = random.choice(GIRL_NAMES)
    update_player(
        user_id,
        has_girlfriend=1,
    ...
    await query.answer(f"✅ Вы нашли девушку!", show_alert=True)
    await query.edit_message_text(f"💕 Поздравляем! Вы нашли девушку - {girlfriend_name}!\n😊 Счастье: 50/100")

async def date_girlfriend(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    
    if not player['has_girlfriend']:
        await query.answer("❌ У вас нет девушки!", show_alert=True)
        return
    
    if player['money'] < 100:
        await query.answer("❌ Нужно 100 руб. на свидание!", show_alert=True)
        return
    
    happiness_increase = random.randint(10, 25)
    new_happiness = min(100, player['girlfriend_happiness'] + happiness_increase)
    
    update_player(
        user_id,
        money=player['money'] - 100,
        girlfriend_happiness=new_happiness,
        happiness=min(100, player['happiness'] + 10)
    )
    
    await query.answer("✅ Свидание прошло успешно!", show_alert=True)
    await query.edit_message_text(
        f"💑 Свидание с {player['girlfriend_name']} прошло отлично!\n\n"
        f"😊 Счастье девушки: +{happiness_increase} (теперь {new_happiness}/100)\n"
        f"💵 Потрачено: 100 руб.\n"
        f"😄 Ваше счастье: +10"
    )

async def gift_girlfriend(query):
    user_id = query.from_user.id
    player = get_player(user_id)
    ...
    if player['money'] < 200:
        await query.answer("❌ Нужно 200 руб. на подарок!", show_alert=True)
        return
    ...
    await query.answer("✅ Подарок понравился!", show_alert=True)
    await query.edit_message_text(
        f"🎁 Вы подарили подарок {player['girlfriend_name']}!\n\n"
        f"😊 Счастье девушки: +{happiness_increase} (теперь {new_happiness}/100)\n"
        f"💵 Потрачено: 200 руб."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏠 Статус":
        ...
        await skills_menu(update, context)
    elif text == "🏥 Здоровье":
