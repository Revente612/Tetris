...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS players (
            ...
            camel_cigarettes INTEGER DEFAULT 0,
            last_sex_time TEXT
        )
    ''')
...
def upgrade_db():
    ...
    try:
        ...
        new_columns = {
            ...
            'camel_cigarettes': 'ALTER TABLE players ADD COLUMN camel_cigarettes INTEGER DEFAULT 0',
            'last_sex_time': 'ALTER TABLE players ADD COLUMN last_sex_time TEXT'
        }
...
def get_player(user_id):
    ...
    if player:
        columns = ['user_id', 'username', 'money', 'health', 'happiness', 'education_level', 
                  ...
                  'gym_level', 'last_gym_time', 'reputation', 'completed_quests', 'camel_cigarettes', 'last_sex_time']
        ...
        defaults = {
            ...
            'gym_level': 0, 'reputation': 0, 'completed_quests': 0, 'camel_cigarettes': 0
        }
...
async def show_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player['camel_cigarettes'] > 0:
        cigarettes_info += f"\n🐫 Camel: {player['camel_cigarettes']} шт."
...
{'🔄 Пора менять картридж вейпа!' if player['vape_puffs_count'] >= 100 else ''}
...
async def school_smoke(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if random.random() < teacher_catch_chance:
        ...
        keyboard = [
            ...
            ["🐫 Выкурить Camel", "💨 Покурить вейп"],
            ["🚬 Покурить одноразку", "🔥 Покурить айкос"],
            ["📦 Закинуть снюс", "🌿 Закинуть безникотиновый снюс"],
...
async def smoke_vape_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        ["🍃 Покурить бумагу с чаем", "⚡ Перезарядить одноразку"],
        ["🔄 Заменить картридж вейпа", "⬅️ Назад"]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def smoke_camel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    ...
    if player['camel_cigarettes'] <= 0:
        await update.message.reply_text("❌ У вас нет Camel! Купите в магазине.")
        return
    
    new_camel = player['camel_cigarettes'] - 1
    new_health = max(0, player['health'] - 16)
    new_happiness = min(100, player['happiness'] + 28)
    
    update_player(
        user_id,
        camel_cigarettes=new_camel,
        health=new_health,
        happiness=new_happiness
    )
    
    await update.message.reply_text(f"🐫 Вы выкурили Camel крепкие...\n\n❤️ Здоровье: -16 (теперь {new_health})\n😊 Счастье: +28 (теперь {new_happiness})\n📦 Camel осталось: {new_camel}")
...
async def vape(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Специальные эффекты для новых жидкостей
    if player['juice_flavor'] == "Анархия хард Виноград лёд":
        # 150mg - очень опасная жидкость
        cancer_chance = 0.1  # 10% шанс "рака" (долгосрочный эффект)
        if random.random() < cancer_chance:
            additional_effects += "\n🦠 ОЙ-ЙОЙ! Похоже у вас развился рак от жидкости! Здоровье будет уменьшаться быстрее!"
        
        cough_chance = 0.8  # 80% шанс кашля
        if random.random() < cough_chance:
            additional_effects += "\n😷 Сильный кашель! Горло разъедает от крепости!"
            health_decrease += 5  # Дополнительный урон

    elif player['juice_flavor'] == "NurikVape Персиковый шот":
        # 90mg - может вызвать потерю сознания
    ...
    elif player['juice_flavor'] == "Skala Банан лёд":
        # 30mg - приятные ощущения
        if random.random() < 0.7:  # 70% шанс
            additional_effects += "\n❄️ Приятный холодок во рту! Очень вкусно!"
    
    # Проверка на протечку Pasito mini
    if player['vape_type'] == "Pasito mini":
        leak_chance = 0.2  # 20% шанс протечки
        if random.random() < leak_chance:
            juice_loss = random.randint(5, 15)
            additional_effects += f"\n💧 ОЙ! Pasito mini протек! Потеряно {juice_loss}мл жидкости!"
            # Уменьшаем жидкость дополнительно
            player['vape_juice'] = max(0, player['vape_juice'] - juice_loss)
    
    # ИСПРАВЛЕНИЕ: теперь используется 1мл жидкости и 2% заряда
    new_juice = max(0, player['vape_juice'] - 1)
    new_battery = max(0, player['vape_battery'] - 2)
    new_health = max(0, player['health'] - health_decrease)
    new_happiness = min(100, player['happiness'] + 25)
...
async def use_snus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Эффект разъедания губы для снюса 1000 мг
    if player['snus_strength'] >= 1000:
        lip_damage_chance = 0.6  # 60% шанс разъедания губы
        if random.random() < lip_damage_chance:
            hospital_time = 60  # 1 минута в больнице
            unconscious_until = datetime.now() + timedelta(seconds=hospital_time)
            update_player(user_id, unconscious_until=unconscious_until.isoformat())
            additional_effects = f"\n💀 ГУБА РАЗЪЕДЕНА! Вы в больнице на {hospital_time} секунд!"
            new_health = max(0, new_health - 30)  # Дополнительный урон здоровью
...
async def use_hookah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Для табаков крепостью 10/10 добавляем специальные эффекты
    if "10/10" in tobacco:
        if random.random() < 0.7:  # 70% шанс головокружения
            additional_effects = "\n🌀 Крепкий табак! Сильное головокружение..."
        if random.random() < 0.3:  # 30% шанс потери сознания
            unconscious_time = random.randint(5, 15)
            unconscious_until = datetime.now() + timedelta(seconds=unconscious_time)
            update_player(user_id, unconscious_until=unconscious_until.isoformat())
            additional_effects += f"\n💫 СЛИШКОМ КРЕПКО! Потеря сознания на {unconscious_time} секунд!"
...
async def smoke_at_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Успешное курение - ИСПРАВЛЕНИЕ: теперь используется 1мл жидкости и 2% заряда
    new_juice = max(0, player['vape_juice'] - 1)
    new_battery = max(0, player['vape_battery'] - 2)
    new_health = max(0, player['health'] - 8)
    new_happiness = min(100, player['happiness'] + 25)
...
# НОВАЯ ФУНКЦИЯ: Позвать девушку в гости
async def invite_girlfriend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    ...
    if not player['has_apartment']:
        await update.message.reply_text("❌ У вас нет своей квартиры! Купите квартиру чтобы приглашать девушку.")
        return
    ...
    keyboard = [
        ["💨 Покурить вейп с девушкой", "🍻 Выпить пиво с девушкой"],
        ["💕 Заняться сексом", "🎬 Посмотреть фильм"],
        ["⬅️ Назад"]
    ]
    reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
    
    await update.message.reply_text("💕 Девушка пришла к вам в гости!\n\nЧто хотите сделать вместе?", reply_markup=reply_markup)

# НОВАЯ ФУНКЦИЯ: Курить вейп с девушкой
async def vape_with_girlfriend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    ...
    if player['vape_battery'] <= 0:
        await update.message.reply_text("🪫 Вейп разряжен!")
        return
    
    if player['vape_juice'] <= 0:
        await update.message.reply_text("❌ В вейпе закончилась жидкость!")
        return
    
    # Используем 2мл жидкости и 4% заряда (вдвоем)
    new_juice = max(0, player['vape_juice'] - 2)
    new_battery = max(0, player['vape_battery'] - 4)
    new_health = max(0, player['health'] - 6)
    new_happiness = min(100, player['happiness'] + 30)
    new_girlfriend_happiness = min(100, player['girlfriend_happiness'] + 20)
    ...
    await update.message.reply_text(f"💨 Вы покурили вейп с девушкой...\n\n❤️ Здоровье: -6 (теперь {new_health})\n😊 Ваше счастье: +30 (теперь {new_happiness})\n💕 Счастье девушки: +20 (теперь {new_girlfriend_happiness})\n💧 Жидкости осталось: {new_juice}мл\n🔋 Батарея вейпа: {new_battery}%")

# НОВАЯ ФУНКЦИЯ: Секс с девушкой
async def have_sex(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    ...
    # Проверка кулдауна (раз в 10 минут)
    if player['last_sex_time']:
        last_sex = datetime.fromisoformat(player['last_sex_time'])
        time_diff = (datetime.now() - last_sex).total_seconds()
        if time_diff < 600:  # 10 минут
            seconds_left = 600 - int(time_diff)
            await update.message.reply_text(f"💕 Вы устали! Подождите {seconds_left} секунд.")
            return
    
    new_happiness = min(100, player['happiness'] + 40)
    new_girlfriend_happiness = min(100, player['girlfriend_happiness'] + 35)
    new_health = max(0, player['health'] - 5)  # Немного усталости
    ...
    update_player(
        ...
        girlfriend_happiness=new_girlfriend_happiness,
        health=new_health,
        last_sex_time=datetime.now().isoformat()
    )
    
    await update.message.reply_text(f"💕 Вы занялись сексом с девушкой...\n\n😊 Ваше счастье: +40 (теперь {new_happiness})\n💕 Счастье девушки: +35 (теперь {new_girlfriend_happiness})\n❤️ Здоровье: -5 (немного устали)")
...
async def girlfriend_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not player['has_girlfriend']:
        ...
        if player['has_apartment']:
            keyboard.append(["🏠 Позвать девушку в гости"])
...
# МАГАЗИН NURIKVAPE - ОБНОВЛЕННАЯ ВЕРСИЯ
async def nurik_vape_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        # Новые вейпы
        [InlineKeyboardButton("💨 Boost 2 black (1200 руб.)", callback_data="buy_boost_2_black")],
        [InlineKeyboardButton("💨 Boost 2 b100 Limit (2300 руб.)", callback_data="buy_boost_2_b100")],
        [InlineKeyboardButton("💨 Charon Baby Pink (600 руб.)", callback_data="buy_charon_baby_pink")],
        [InlineKeyboardButton("💨 Minican mini green (320 руб.)", callback_data="buy_minican_mini_green")],
        [InlineKeyboardButton("💨 Xros 3 mini (1200 руб.)", callback_data="buy_xros_3_mini_new")],
        [InlineKeyboardButton("💨 Pasito mini - МОЖЕТ ПРОТЕЧЬ (2200 руб.)", callback_data="buy_pasito_mini")],
        [InlineKeyboardButton("💨 Pasito 2 - ОЧЕНЬ МОЩНЫЙ (3300 руб.)", callback_data="buy_pasito_2")],
        
        # Новые жидкости
        [InlineKeyboardButton("💧 Анархия хард Виноград лёд 150mg - ОЧЕНЬ ОПАСНО! (800 руб.)", callback_data="buy_anarchy_hard_grape")],
        [InlineKeyboardButton("💧 Анархия Виноград лёд 40mg (500 руб.)", callback_data="buy_anarchy_grape")],
        [InlineKeyboardButton("💧 Dual Кола 3mg (400 руб.)", callback_data="buy_dual_cola")],
        [InlineKeyboardButton("💧 NurikVape Персиковый шот 90mg (700 руб.)", callback_data="buy_nurikvape_peach")],
        [InlineKeyboardButton("💧 Skala Банан лёд 30mg (450 руб.)", callback_data="buy_skala_banana")],
        
        # Старые товары (оставлены)
        [InlineKeyboardButton("💨 Hero 3 Pink Limit (6600 руб.)", callback_data="buy_hero_3_pink")],
        [InlineKeyboardButton("💧 Dual Земляника-Клубника-Снег 30mg (500 руб.)", callback_data="buy_dual_strawberry")],
        [InlineKeyboardButton("🐫 Camel крепкие (20 шт.) (500 руб.)", callback_data="buy_camel")],
        [InlineKeyboardButton("📦 Снюс Siberia 1000mg (800 руб.)", callback_data="buy_siberia_snus")],
        [InlineKeyboardButton("📦 Снюс Odens 1000mg (750 руб.)", callback_data="buy_odens_snus")],
        [InlineKeyboardButton("📦 Снюс Thunder 1000mg (700 руб.)", callback_data="buy_thunder_snus")],
        [InlineKeyboardButton("🌿 Табак DarkSide Суперкрепкий 10/10 (800 руб.)", callback_data="buy_darkside_strong")],
        [InlineKeyboardButton("🌿 Табак MustHave Ледяная мята 10/10 (850 руб.)", callback_data="buy_musthave_mint")],
        [InlineKeyboardButton("🌿 Табак Spectrum Крем-брюле 10/10 (820 руб.)", callback_data="buy_spectrum_cream")],
        [InlineKeyboardButton("🌿 Табак Element Виноградный лед 10/10 (780 руб.)", callback_data="buy_element_grape")],
        [InlineKeyboardButton("🌿 Табак DailyHookah Апельсин-шоколад 10/10 (790 руб.)", callback_data="buy_daily_orange")],
        [InlineKeyboardButton("⬅️ Назад в магазин", callback_data="back_to_shop")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await query.edit_message_text(
        "🛒 Магазин NurikVape - Расширенный ассортимент\n\n"
        "💨 НОВЫЕ ВЕЙПЫ:\n"
        "• Boost 2 black - 1200 руб.\n"
        "• Boost 2 b100 Limit - 2300 руб.\n" 
        "• Charon Baby Pink - 600 руб.\n"
        "• Minican mini green - 320 руб.\n"
        "• Xros 3 mini - 1200 руб.\n"
        "• Pasito mini - МОЖЕТ ПРОТЕЧЬ! - 2200 руб.\n"
        "• Pasito 2 - ОЧЕНЬ МОЩНЫЙ - 3300 руб.\n\n"
        
        "💧 НОВЫЕ ЖИДКОСТИ:\n"
        "• Анархия хард Виноград лёд 150mg - ОЧЕНЬ ОПАСНО! - 800 руб.\n"
        "• Анархия Виноград лёд 40mg - 500 руб.\n"
        "• Dual Кола 3mg - 400 руб.\n"
        "• NurikVape Персиковый шот 90mg - 700 руб.\n"
        "• Skala Банан лёд 30mg - 450 руб.\n\n"
        
        "💨 ПРЕМИУМ ВЕЙПЫ:\n"
        "• Hero 3 Pink Limit - 6600 руб.\n\n"
        
        "💧 ЖИДКОСТИ:\n"
        "• Dual Земляника-Клубника-Снег 30mg - 500 руб.\n\n"
        
        "🚬 СИГАРЕТЫ:\n"
        "• Camel крепкие (20 шт.) - 500 руб.\n\n"
        
        "📦 СНЮС (ОПАСНО! 1000 мг):\n"
        "• Siberia 1000mg - 800 руб.\n"
        "• Odens 1000mg - 750 руб.\n"
        "• Thunder 1000mg - 700 руб.\n\n"
        
        "🌿 ТАБАКИ ДЛЯ КАЛЬЯНА (10/10 крепость):\n"
        "• DarkSide Суперкрепкий - 800 руб.\n"
        "• MustHave Ледяная мята - 850 руб.\n"
        "• Spectrum Крем-брюле - 820 руб.\n"
        "• Element Виноградный лед - 780 руб.\n"
        "• DailyHookah Апельсин-шоколад - 790 руб.\n\n"
        
        "⚠️ Внимание! Некоторые товары могут быть опасны для здоровья!",
...
async def handle_shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif callback_data == "charge_vape":
        await charge_vape(query, player)
    elif callback_data == "charge_iqos":
        await charge_iqos(query, player)

# ФУНКЦИЯ ЗАРЯДКИ ВЕЙПА
async def charge_vape(query, player):
    user_id = query.from_user.id
    
    if not player['vape_type']:
        await query.edit_message_text("❌ У вас нет вейпа!")
        return
    
    if player['money'] < 50:
        await query.edit_message_text("❌ Недостаточно денег для зарядки вейпа! Нужно 50 руб.")
        return
    
    new_money = player['money'] - 50
    new_battery = 100  # Полная зарядка
    ...
    await query.edit_message_text(f"🔋 Зарядили вейп!\n\n💵 Потрачено: 50 руб.\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")

# ФУНКЦИЯ ЗАРЯДКИ АЙКОС
async def charge_iqos(query, player):
    user_id = query.from_user.id
    
    if not player['has_iqos']:
        await query.edit_message_text("❌ У вас нет Айкос!")
        return
    
    if player['money'] < 100:
        await query.edit_message_text("❌ Недостаточно денег для зарядки Айкос! Нужно 100 руб.")
        return
    ...
    await query.edit_message_text(f"🔋 Зарядили Айкос!\n\n💵 Потрачено: 100 руб.\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
...
# ОСНОВНАЯ ФУНКЦИЯ ОБРАБОТКИ ПОКУПОК С ИСПРАВЛЕНИЯМИ
async def handle_purchase(query, callback_data, player):
    ...
    # НОВЫЕ ВЕЙПЫ ИЗ NURIKVAPE
    elif callback_data == "buy_boost_2_black":
        if player['money'] >= 1200:
            new_money = player['money'] - 1200
            update_player(user_id, money=new_money, vape_type="Boost 2 black", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Boost 2 black!\n\n💵 Потрачено: 1200 руб.\n💨 Теперь у вас есть новый вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Boost 2 black!")
            return

    elif callback_data == "buy_boost_2_b100":
        if player['money'] >= 2300:
            new_money = player['money'] - 2300
            update_player(user_id, money=new_money, vape_type="Boost 2 b100 Limit", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Boost 2 b100 Limit!\n\n💵 Потрачено: 2300 руб.\n💨 Теперь у вас есть лимитированный вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Boost 2 b100 Limit!")
            return

    elif callback_data == "buy_charon_baby_pink":
        if player['money'] >= 600:
            new_money = player['money'] - 600
            update_player(user_id, money=new_money, vape_type="Charon Baby Pink", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Charon Baby Pink!\n\n💵 Потрачено: 600 руб.\n💨 Теперь у вас есть розовый вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Charon Baby Pink!")
            return

    elif callback_data == "buy_minican_mini_green":
        if player['money'] >= 320:
            new_money = player['money'] - 320
            update_player(user_id, money=new_money, vape_type="Minican mini green", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Minican mini green!\n\n💵 Потрачено: 320 руб.\n💨 Теперь у вас есть мини-вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Minican mini green!")
            return

    elif callback_data == "buy_xros_3_mini_new":
        if player['money'] >= 1200:
            new_money = player['money'] - 1200
            update_player(user_id, money=new_money, vape_type="Xros 3 mini", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Xros 3 mini!\n\n💵 Потрачено: 1200 руб.\n💨 Теперь у вас есть компактный вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Xros 3 mini!")
            return

    elif callback_data == "buy_pasito_mini":
        if player['money'] >= 2200:
            new_money = player['money'] - 2200
            update_player(user_id, money=new_money, vape_type="Pasito mini", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Pasito mini!\n\n💵 Потрачено: 2200 руб.\n💨 Теперь у вас есть Pasito mini!\n⚠️ Внимание: этот вейп может протечь!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Pasito mini!")
            return

    elif callback_data == "buy_pasito_2":
        if player['money'] >= 3300:
            new_money = player['money'] - 3300
            update_player(user_id, money=new_money, vape_type="Pasito 2", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Pasito 2!\n\n💵 Потрачено: 3300 руб.\n💨 Теперь у вас есть ОЧЕНЬ МОЩНЫЙ вейп!\n⚡ Подходит для сильных жидкостей\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Pasito 2!")
            return

    # НОВЫЕ ЖИДКОСТИ ИЗ NURIKVAPE
    elif callback_data == "buy_anarchy_hard_grape":
        if player['money'] >= 800:
            new_money = player['money'] - 800
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="Анархия хард Виноград лёд", juice_strength=150)
            await query.edit_message_text(f"💧 Купили Анархия хард Виноград лёд 150mg!\n\n💵 Потрачено: 800 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n☠️ ОЧЕНЬ ОПАСНО! Может вызвать рак и сильный кашель!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return

    elif callback_data == "buy_anarchy_grape":
        if player['money'] >= 500:
            new_money = player['money'] - 500
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="Анархия Виноград лёд", juice_strength=40)
            await query.edit_message_text(f"💧 Купили Анархия Виноград лёд 40mg!\n\n💵 Потрачено: 500 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n⚠️ Средняя крепость, может вызвать кашель\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return

    elif callback_data == "buy_dual_cola":
        if player['money'] >= 400:
            new_money = player['money'] - 400
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="Dual Кола", juice_strength=3)
            await query.edit_message_text(f"💧 Купили Dual Кола 3mg!\n\n💵 Потрачено: 400 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n😊 Средняя крепость, приносит удовольствие\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return

    elif callback_data == "buy_nurikvape_peach":
        if player['money'] >= 700:
            new_money = player['money'] - 700
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="NurikVape Персиковый шот", juice_strength=90)
            await query.edit_message_text(f"💧 Купили NurikVape Персиковый шот 90mg!\n\n💵 Потрачено: 700 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n⚠️ Крепкая! Может вызвать потерю сознания на 10 секунд\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return

    elif callback_data == "buy_skala_banana":
        if player['money'] >= 450:
            new_money = player['money'] - 450
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="Skala Банан лёд", juice_strength=30)
            await query.edit_message_text(f"💧 Купили Skala Банан лёд 30mg!\n\n💵 Потрачено: 450 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n❄️ Очень вкусно! Вызывает холод во рту и приятные ощущения\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return
    ...
    # ТОВАРЫ ИЗ NURIKVAPE
    elif callback_data == "buy_xros_3":
        if player['money'] >= 4500:
            new_money = player['money'] - 4500
            update_player(user_id, money=new_money, vape_type="Xros 3 Mini", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Xros 3 Mini!\n\n💵 Потрачено: 4500 руб.\n💨 Теперь у вас есть премиум вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Xros 3!")
            return
    
    elif callback_data == "buy_hero_3_pink":
        if player['money'] >= 6600:
            new_money = player['money'] - 6600
            update_player(user_id, money=new_money, vape_type="Hero 3 Pink Limit", vape_battery=100)
            await query.edit_message_text(f"💨 Купили Hero 3 Pink Limit!\n\n💵 Потрачено: 6600 руб.\n💨 Теперь у вас есть эксклюзивный вейп!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Hero 3 Pink Limit!")
            return
    
    elif callback_data == "buy_dual_strawberry":
        if player['money'] >= 500:
            new_money = player['money'] - 500
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="Dual Земляника-Клубника-Снег", juice_strength=30)
            await query.edit_message_text(f"💧 Купили Dual Земляника-Клубника-Снег 30mg!\n\n💵 Потрачено: 500 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n🍓 Очень вкусная жидкость!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return
    
    elif callback_data == "buy_camel":
        if player['money'] >= 500:
            new_money = player['money'] - 500
            new_camel = player['camel_cigarettes'] + 20
            update_player(user_id, money=new_money, camel_cigarettes=new_camel)
            await query.edit_message_text(f"🐫 Купили Camel крепкие!\n\n💵 Потрачено: 500 руб.\n🐫 Camel: +20 (теперь {new_camel})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Camel!")
            return
    
    # СНЮС 1000 мг (опасный)
    elif callback_data == "buy_siberia_snus":
        if player['money'] >= 800:
            new_money = player['money'] - 800
            new_snus = player['snus_packs'] + 1
            update_player(user_id, money=new_money, snus_packs=new_snus, snus_flavor="Siberia", snus_strength=1000)
            await query.edit_message_text(f"📦 Купили Siberia 1000mg!\n\n💵 Потрачено: 800 руб.\n📦 Снюс: +1 пачка (теперь {new_snus})\n⚠️ ОПАСНО! 1000 мг может разъесть губу!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Siberia!")
            return
    
    elif callback_data == "buy_odens_snus":
        if player['money'] >= 750:
            new_money = player['money'] - 750
            new_snus = player['snus_packs'] + 1
            update_player(user_id, money=new_money, snus_packs=new_snus, snus_flavor="Odens", snus_strength=1000)
            await query.edit_message_text(f"📦 Купили Odens 1000mg!\n\n💵 Потрачено: 750 руб.\n📦 Снюс: +1 пачка (теперь {new_snus})\n⚠️ ОПАСНО! 1000 мг может разъесть губу!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Odens!")
            return
    
    elif callback_data == "buy_thunder_snus":
        if player['money'] >= 700:
            new_money = player['money'] - 700
            new_snus = player['snus_packs'] + 1
            update_player(user_id, money=new_money, snus_packs=new_snus, snus_flavor="Thunder", snus_strength=1000)
            await query.edit_message_text(f"📦 Купили Thunder 1000mg!\n\n💵 Потрачено: 700 руб.\n📦 Снюс: +1 пачка (теперь {new_snus})\n⚠️ ОПАСНО! 1000 мг может разъесть губу!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Thunder!")
            return
    
    # ТАБАКИ ДЛЯ КАЛЬЯНА 10/10
    elif callback_data == "buy_darkside_strong":
        if player['money'] >= 800:
            new_money = player['money'] - 800
            update_player(user_id, money=new_money, hookah_tobacco="DarkSide Суперкрепкий 10/10", hookah_tobacco_amount=50)
            await query.edit_message_text(f"🌿 Купили DarkSide Суперкрепкий 10/10!\n\n💵 Потрачено: 800 руб.\n🌿 Табак: 50г\n💪 Крепость: 10/10 - очень крепкий!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки табака!")
            return
    
    elif callback_data == "buy_musthave_mint":
        if player['money'] >= 850:
            new_money = player['money'] - 850
            update_player(user_id, money=new_money, hookah_tobacco="MustHave Ледяная мята 10/10", hookah_tobacco_amount=50)
            await query.edit_message_text(f"🌿 Купили MustHave Ледяная мята 10/10!\n\n💵 Потрачено: 850 руб.\n🌿 Табак: 50г\n💪 Крепость: 10/10 - очень крепкий!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки табака!")
            return
    
    elif callback_data == "buy_spectrum_cream":
        if player['money'] >= 820:
            new_money = player['money'] - 820
            update_player(user_id, money=new_money, hookah_tobacco="Spectrum Крем-брюле 10/10", hookah_tobacco_amount=50)
            await query.edit_message_text(f"🌿 Купили Spectrum Крем-брюле 10/10!\n\n💵 Потрачено: 820 руб.\n🌿 Табак: 50г\n💪 Крепость: 10/10 - очень крепкий!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки табака!")
            return
    
    elif callback_data == "buy_element_grape":
        if player['money'] >= 780:
            new_money = player['money'] - 780
            update_player(user_id, money=new_money, hookah_tobacco="Element Виноградный лед 10/10", hookah_tobacco_amount=50)
            await query.edit_message_text(f"🌿 Купили Element Виноградный лед 10/10!\n\n💵 Потрачено: 780 руб.\n🌿 Табак: 50г\n💪 Крепость: 10/10 - очень крепкий!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки табака!")
            return
    
    elif callback_data == "buy_daily_orange":
        if player['money'] >= 790:
            new_money = player['money'] - 790
            update_player(user_id, money=new_money, hookah_tobacco="DailyHookah Апельсин-шоколад 10/10", hookah_tobacco_amount=50)
            await query.edit_message_text(f"🌿 Купили DailyHookah Апельсин-шоколад 10/10!\n\n💵 Потрачено: 790 руб.\n🌿 Табак: 50г\n💪 Крепость: 10/10 - очень крепкий!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки табака!")
            return
    ...
    # ИСПРАВЛЕННЫЕ ПОКУПКИ КАЛЬЯНА И АЙКОС
    elif callback_data == "buy_hookah":
        if player['money'] >= 1500:
            new_money = player['money'] - 1500
            update_player(user_id, money=new_money, has_hookah=1)
            await query.edit_message_text(f"💨 Купили кальян!\n\n💵 Потрачено: 1500 руб.\n💨 Теперь у вас есть кальян!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки кальяна!")
            return

    elif callback_data == "buy_iqos":
        if player['money'] >= 2000:
            new_money = player['money'] - 2000
            update_player(user_id, money=new_money, has_iqos=1, iqos_battery=100)
            await query.edit_message_text(f"🔥 Купили Айкос!\n\n💵 Потрачено: 2000 руб.\n🔥 Теперь у вас есть Айкос!\n🔋 Батарея: 100%\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Айкос!")
            return

    elif callback_data == "buy_heets":
        if player['money'] >= 400:
            new_money = player['money'] - 400
            new_sticks = player['iqos_sticks'] + 20
            update_player(user_id, money=new_money, iqos_sticks=new_sticks, stick_flavor="HEETS обычные")
            await query.edit_message_text(f"📦 Купили стики HEETS!\n\n💵 Потрачено: 400 руб.\n📦 Стиков: +20 (теперь {new_sticks})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки стиков!")
            return

    elif callback_data == "buy_coals":
        if player['money'] >= 100:
            new_money = player['money'] - 100
            new_coals = player['hookah_coals'] + 10
            update_player(user_id, money=new_money, hookah_coals=new_coals)
            await query.edit_message_text(f"🔥 Купили угли для кальяна!\n\n💵 Потрачено: 100 руб.\n🔥 Углей: +10 (теперь {new_coals})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки углей!")
            return

    elif callback_data == "buy_burner":
        if player['money'] >= 200:
            new_money = player['money'] - 200
            update_player(user_id, money=new_money, has_burner=1)
            await query.edit_message_text(f"🔥 Купили горелку!\n\n💵 Потрачено: 200 руб.\n🔥 Теперь у вас есть горелка для углей!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки горелки!")
            return

    elif callback_data == "buy_darkside_tobacco":
        if player['money'] >= 600:
            new_money = player['money'] - 600
            update_player(user_id, money=new_money, hookah_tobacco="DarkSide", hookah_tobacco_amount=50)
            await query.edit_message_text(f"🌿 Купили табак DarkSide!\n\n💵 Потрачено: 600 руб.\n🌿 Табак: 50г\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки табака!")
            return

    elif callback_data == "buy_snus":
        if player['money'] >= 300:
            new_money = player['money'] - 300
            new_snus = player['snus_packs'] + 1
            update_player(user_id, money=new_money, snus_packs=new_snus, snus_flavor="Обычный", snus_strength=150)
            await query.edit_message_text(f"📦 Купили снюс!\n\n💵 Потрачено: 300 руб.\n📦 Снюс: +1 пачка (теперь {new_snus})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки снюса!")
            return

    elif callback_data == "buy_strong_snus":
        if player['money'] >= 500:
            new_money = player['money'] - 500
            new_snus = player['snus_packs'] + 1
            update_player(user_id, money=new_money, snus_packs=new_snus, snus_flavor="Крепкий", snus_strength=500)
            await query.edit_message_text(f"📦 Купили крепкий снюс!\n\n💵 Потрачено: 500 руб.\n📦 Снюс: +1 пачка (теперь {new_snus})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки снюса!")
            return

    elif callback_data == "buy_nicotine_free_snus":
        if player['money'] >= 200:
            new_money = player['money'] - 200
            update_player(user_id, money=new_money, has_nicotine_free_snus=1)
            await query.edit_message_text(f"🌿 Купили безникотиновый снюс!\n\n💵 Потрачено: 200 руб.\n🌿 Теперь у вас есть безникотиновый снюс!\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки безникотинового снюса!")
            return

    elif callback_data == "buy_nicotine_free_juice":
        if player['money'] >= 250:
            new_money = player['money'] - 250
            new_juice = player['vape_juice'] + 30
            update_player(user_id, money=new_money, vape_juice=new_juice, juice_flavor="Безникотиновая", juice_strength=0)
            await query.edit_message_text(f"💧 Купили безникотиновую жидкость!\n\n💵 Потрачено: 250 руб.\n💧 Жидкости: +30мл (теперь {new_juice}мл)\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки жидкости!")
            return

    elif callback_data == "buy_disposable_hqd":
        if player['money'] >= 800:
            new_money = player['money'] - 800
            update_player(user_id, money=new_money, disposable_vape_type="HQD", disposable_vape_puffs=1500, disposable_vape_defective=0)
            await query.edit_message_text(f"🚬 Купили HQD одноразку!\n\n💵 Потрачено: 800 руб.\n📦 Тяг: 1500\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки HQD!")
            return

    elif callback_data == "buy_disposable_elfbar":
        if player['money'] >= 1000:
            new_money = player['money'] - 1000
            update_player(user_id, money=new_money, disposable_vape_type="Elf Bar", disposable_vape_puffs=2000, disposable_vape_defective=0)
            await query.edit_message_text(f"🚬 Купили Elf Bar одноразку!\n\n💵 Потрачено: 1000 руб.\n📦 Тяг: 2000\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Elf Bar!")
            return

    elif callback_data == "buy_winston":
        if player['money'] >= 400:
            new_money = player['money'] - 400
            new_winston = player['winston_cigarettes'] + 20
            update_player(user_id, money=new_money, winston_cigarettes=new_winston)
            await query.edit_message_text(f"🚬 Купили Winston!\n\n💵 Потрачено: 400 руб.\n🚬 Winston: +20 (теперь {new_winston})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Winston!")
            return

    elif callback_data == "buy_parliament":
        if player['money'] >= 450:
            new_money = player['money'] - 450
            new_parliament = player['parliament_cigarettes'] + 20
            update_player(user_id, money=new_money, parliament_cigarettes=new_parliament)
            await query.edit_message_text(f"🚬 Купили Parliament!\n\n💵 Потрачено: 450 руб.\n🚬 Parliament: +20 (теперь {new_parliament})\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        else:
            await query.edit_message_text("❌ Недостаточно денег для покупки Parliament!")
            return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text == "🐫 Выкурить Camel":
        await smoke_camel(update, context)
    ...
    elif text == "🏠 Позвать девушку в гости":
        await invite_girlfriend(update, context)
    elif text == "💨 Покурить вейп с девушкой":
        await vape_with_girlfriend(update, context)
    elif text == "💕 Заняться сексом":
        await have_sex(update, context)
