...
async def handle_purchase(query, callback_data, player):
    ...
    elif callback_data == "get_job_1":
        if player['job_level'] == 0:
            update_player(user_id, job_level=1)
            await query.edit_message_text(f"💼 Устроились разнорабочим!\n\n💰 Зарплата: 50 руб. за работу\n💼 Теперь вы можете работать!")
            purchase_made = True
        else:
            await query.edit_message_text("❌ У вас уже есть работа!")
    
    elif callback_data == "get_job_2":
        if player['money'] >= 500 and player['job_level'] < 2:
            new_money = player['money'] - 500
            update_player(user_id, money=new_money, job_level=2)
            await query.edit_message_text(f"💼 Устроились продавцом!\n\n💵 Потрачено: 500 руб.\n💰 Зарплата: 100 руб. за работу\n💵 Осталось: {new_money} руб.")
            purchase_made = True
        elif player['job_level'] >= 2:
            await query.edit_message_text("❌ У вас уже есть работа этого уровня или выше!")
        else:
            await query.edit_message_text("❌ Недостаточно денег для устройства на работу!")
    
    # Добавьте обработчики для других товаров по аналогии...
    
    if not purchase_made and not callback_data.startswith("get_job"):
